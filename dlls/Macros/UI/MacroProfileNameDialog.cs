...
{
    internal sealed class MacroProfileNameDialog : Form
    {
        ...
        private readonly IReadOnlyCollection<MacroProfile> _profiles;

        public MacroProfileNameDialog(IReadOnlyCollection<MacroProfile> profiles, string suggestedName)
        {
            _profiles = profiles;
        ...
        private void TryAccept()
        {
            ...
            if (!MacroSettingsHelper.IsBuildProfileNameAvailable(name, _profiles))
            {
                _errorLabel.Text = string.IsNullOrWhiteSpace(name)
                    ? "Name is required."
                    : "That name is already in use or reserved.";
